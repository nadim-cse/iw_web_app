...
<div id="sideBar" class="relative flex flex-col flex-wrap bg-white border-r border-gray-300 p-6 flex-none w-64 md:-ml-64 md:fixed md:top-0  md:z-30 md:h-screen md:shadow-xl animated faster">
    ...
    <div class="flex flex-col">
        ...
        @if($item['is_dropdown'] == 1) 
            <p class="uppercase text-xs text-gray-600 mb-4 tracking-wider">{{ $item['title']}}</p>
        @endif
        <?php $route = $item['route']; ?>
         <!-- link -->
         <a href="{{ route($route) }}" class="mb-3 capitalize font-medium text-sm hover:text-teal-600 transition ease-in-out duration-500">
            <i class="{{ $item['icon'] }} text-xs mr-2"></i>
           {{ $item['name'] }}
        </a>
        @endforeach
        @endif
        ...
        <!-- <a href="#" class="mb-3 capitalize font-medium text-sm hover:text-teal-600 transition ease-in-out duration-500"  aria-controls="dropdown-example" data-collapse-toggle="dropdown-example">
